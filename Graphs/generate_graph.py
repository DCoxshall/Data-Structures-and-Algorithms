...
def generate_graph(n, bi_directional=True, locations=False):
    ...
    for node in graph:
        ...
        four_closest = {k: v for k, v in sorted(
            four_closest.items(), key=lambda item: item[1])}
        ...
        for i in range(0, number_of_connections):
            graph[node][closest_nodes[i]] = math.floor(
                four_closest[closest_nodes[i]])
            if bi_directional:
                graph[closest_nodes[i]][node] = math.floor(
                    four_closest[closest_nodes[i]])
